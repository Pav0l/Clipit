...
/**
 * NOTE: This file is a clone of the Zora AuctionHouse.sol contract.
 * It was cloned from https://github.com/ourzora/auction-house at commit 01c4e8085c6815bf3233057dee8e628aca07813f.
 *
 * The following functions needed to be modified:
 *  - changed solidity version
 *  - changed imports for IMarket and Decimal to import local contracts
 *  - imported IClipIt interface instead of the original IMedia interface
 */
...
import { IClipIt } from "./interfaces/IClipIt.sol";
...
interface IClipItExtended is IClipIt {
...
contract AuctionHouse is IAuctionHouse, ReentrancyGuard {
  ...
  constructor(address _zora, address _weth) public {
    require(IERC165(_zora).supportsInterface(interfaceId), "Doesn't support NFT interface");
    zora = _zora;
    wethAddress = _weth;
    ...
    nonReentrant
  {
    ...
    require(
      amount >= auctions[auctionId].amount.add(auctions[auctionId].amount.mul(minBidIncrementPercentage).div(100)),
    ...
    if (auctions[auctionId].tokenContract == zora) {
      require(
        IMarket(IClipItExtended(zora).marketContract()).isValidBid(auctions[auctionId].tokenId, amount),
  ...
  function _handleZoraAuctionSettlement(uint256 auctionId) internal returns (bool, uint256) {
    ...
    IClipIt(zora).setBid(auctions[auctionId].tokenId, bid);
    uint256 beforeBalance = IERC20(currency).balanceOf(address(this));
    try IClipIt(zora).acceptBid(auctions[auctionId].tokenId, bid) {} catch {
      // If the underlying NFT transfer here fails, we should cancel the auction and refund the winner
      IClipItExtended(zora).removeBid(auctions[auctionId].tokenId);
      return (false, 0);
    }
