...
export function handleReserveAuctionBid(event: AuctionBid): void {
  ...
  if (event.params.firstBid) {
    ...
    handleBidReplaced(auctionCurrentBidId, event.block.timestamp, event.block.number);
  }
...
export function handleReserveAuctionEnded(event: AuctionEnded): void {
  ...
  handleBidReplaced(auctionCurrentBidId, event.block.timestamp, event.block.number, true);

  // Then, finalize the auction
  handleFinishedAuction(auction, event.block.timestamp, event.block.number);
...
export function handleReserveAuctionCanceled(event: AuctionCanceled): void {
  ...
  // Then, finalize the auction
  handleFinishedAuction(auction, event.block.timestamp, event.block.number, true);
